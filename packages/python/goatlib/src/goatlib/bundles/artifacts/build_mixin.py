...
class BundleArtifactBuildMixin:
    ...
    if TYPE_CHECKING:
        ...
        def get_layer_table_path(self, user_id: str, layer_id: str) -> str: ...
    ...
    def export_member_layers(
        ...
        running event loop, and none of its filtering or scenario merging
        applies here. The owner is known from the bundle.
        """
        if not members:
            raise ValueError(
        ...
        for member in members:
            role = member["role"]
            table = self.get_layer_table_path(user_id, str(member["layer_id"]))
